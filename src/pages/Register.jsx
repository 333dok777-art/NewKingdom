...
import { registrationErrorMessage, startRegistration } from '../lib/api.js'
...
function RegisterContent() {
  ...
  const submit = async (event) => { event.preventDefault(); setTouched({ username: true, email: true, password: true, confirmPassword: true, terms: true }); if (!valid) return; setIsSubmitting(true); setServerError(''); try { const result = await startRegistration({ ...values, termsAccepted: values.terms, preferredLanguage: language }); navigate(result.verificationRequired === false ? '/coming-soon' : '/verify-email') } catch (error) { setServerError(registrationErrorMessage(error, language)) } finally { setIsSubmitting(false) } }
